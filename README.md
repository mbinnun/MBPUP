...
**Resolution:**<br />Run **gnome-disks** from terminal. When the GUI shows up, select the swap partition. On its settings menu click on **edit mount options**. On mount options, turn off the "Automatic Mount Options" switch and make sure the "Mount at startup" checkbox is checked. Click OK, then reboot. done, the swap partition will automatically mount.
